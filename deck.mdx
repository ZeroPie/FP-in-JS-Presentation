...
```js title='First Class Functions' subtitle='Können wie andere Datatypen behandelt werden'
  
  const sayHiTo = name => `Hi ${name}`;
  const arr = [sayHiTo]
  const yellHiTo = yell(sayHi)
  greetings = { 
    sayHiTo,
    yellHiTo
  }
...
```markdown title='Side Effects'
  Any application state change that is 

...
```js title='Functions are first class in Javascript' subtitle='Können wie andere Datatypen behandelt werden'
...
```js
Die map() Methode wendet auf jedes Element des Arrays die 
bereitgestellte Funktion an und gibt das Ergebnis in einem neuen Array zurück.
```

```js
const doubledItems = arr.map(v => v * 2)
```
...
```js 
const compose = (f1, f2, f3) => (f1,f2) => f3(f2(f1))
...
```js
pipe(add, mult, div) // right to left
compose(div, mult, add) // left to right
```
